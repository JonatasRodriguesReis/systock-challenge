  ...
  <v-container fluid class="pa-6">
      ...
      <div class="d-flex justify-space-between align-center">
        <div>
          ...
          <div class="text-h4 font-weight-bold">
            {{ myProductStore.totalProducts }} Meus produtos
          </div>
        </div>
        <v-btn
          color="white"
    ...
    <v-card border flat>
      ...
      <v-data-table-server
        ...
        :headers="headers"
        :items="myProductStore.products"
        :items-length="myProductStore.paginationMeta?.total || 0"
        :loading="myProductStore.loading"
        :search="search"
        :page="page"
      ...
      >
        <template v-slot:item.actions="{ item }">
          ...
          <v-icon size="small" color="secondary" @click="confirmDelete(item)">
            mdi-delete
          </v-icon>
        </template>
        ...
        <template v-slot:footer>
          <v-pagination
            v-model="page"
            :length="myProductStore.paginationMeta?.last_page || 1"
            :total-visible="7"
    ...
    <v-dialog v-model="dialog" max-width="600px" persistent>
      <v-card>
        <v-card-title class="pa-4 bg-primary text-white">
          {{ editedItem.id ? "Editar Produto" : "Novo Produto" }}
        </v-card-title>
    ...
    <v-dialog v-model="dialogDelete" max-width="400px">
      <v-card class="pa-4 text-center">
        ...
        >
        <div class="text-h6 mb-2">Deseja excluir este produto?</div>
...
const rules = {
  ...
  preco: [
    (v) => !!v || "Obrigatório",
    (v) => !isNaN(parseFloat(v)) || "Deve ser um número",
...
const loadItems = ({ page, itemsPerPage, sortBy }) => {
  myProductStore.fetchMyProducts({
    page,
...
const save = async () => {
  ...
  saving.value = true;
  try {
    await myProductStore.saveProduct(editedItem);
    dialog.value = false;
    resetPagination();
    myProductStore.fetchMyProducts({
      page: page.value,
...
const deleteItemConfirm = async () => {
  deleting.value = true;
  try {
    await myProductStore.deleteProduct(editedItem.id);
